    ...
    async def verify_member(
        ...
        async with session_factory() as session:
            if guild_settings := await get_guild_settings(session, guild_id=inter.guild_id):
                if support_role := inter.guild.get_role(guild_settings.support_role_id) or inter.author.guild_permissions.administrator:
                        ...
                        if not user.is_verified:
                            if gender_role_id := {
                                ...
                                if gender_role := inter.guild.get_role(gender_role_id):
                                    ...
                                    try:
                                        await member.send(
                                            embed=YouSuccessfullyVerifiedEmbed(support=inter.author),
                                            view=SendSupportFeedbackView(
                                                support_id=inter.author.id,
                                                author_id=member.id,
                                                guild=inter.guild,
                                            ),
                                        )
